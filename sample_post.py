...
def sample_reddit_post(url, output_file, last_output_filename):
    ...
    for comment in response[1]['data']['children']:
        ...
        comment_time = datetime.datetime.strptime(comment_time, DATETIME_FORMAT)
...
def sample_yt_vid(video_id, output_file, last_output_filename):
    ...
    try:
        ...
        if k > 0:
            top_k_indices = list(np.argsort(n_likes)[-k:])
    ...
    except Exception as e:  # typically because the post has disabled comments
        print(f"{video_id}: {e}")
